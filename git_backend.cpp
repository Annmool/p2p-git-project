...
bool GitBackend::commitChanges(const std::string &message, const std::string &author_name, const std::string &author_email, std::string &error_message)
{
    ...
    if (git_repository_index(&index, m_currentRepo) != 0)
    {
        error_message = "Could not get repository index.";
        cleanup();
        return false;
    }
    if (git_index_write_tree(&tree_oid, index) != 0)
    {
        error_message = "Could not write index to tree.";
        cleanup();
        return false;
    }
    if (git_tree_lookup(&tree, m_currentRepo, &tree_oid) != 0)
    {
        error_message = "Could not look up newly created tree.";
        cleanup();
        return false;
    }
    if (git_signature_now(&author, author_name.c_str(), author_email.c_str()) != 0)
    {
        error_message = "Could not create commit signature.";
        cleanup();
        return false;
    }
...
bool GitBackend::createDiffArchive(const std::string &output_zip_path, const std::string &local_branch, const std::string &remote_branch_base, std::string &error_message)
{
    if (!isRepositoryOpen())
    {
        error_message = "No repository open.";
    ...
    // Use a unique, manually managed temporary directory
    ...
    QString fileListStr = listFilesProcess.readAllStandardOutput();
    QStringList files = fileListStr.split('\n', Qt::SkipEmptyParts);

    if (files.isEmpty())
    {
        error_message = "No new commits or changes to propose.";
        tempDir.removeRecursively();
        return false;
    }
    ...
    for (const QString &file : files)
    {
        ...
        // Ensure subdirectories for the diff file exist
    ...
    // Now zip the contents of the temp directory as plain-text .diff files.
    // Many viewers can open .zip directly; ensure the target path is fresh.
    QString outZipPath = QString::fromStdString(output_zip_path);
    if (QFile::exists(outZipPath))
    {
        // Remove any pre-created placeholder (e.g., from QTemporaryFile) to avoid
        // 'Not a zip file' errors when zip tries to update an invalid archive.
        QFile::remove(outZipPath);
    }

    // Run zip from the parent of the temp dir so archive contains the directory root
    // (keeps file paths readable). Alternatively, we could set CWD to tempDir and zip '.'
    // to place files at the root of the archive; current approach names the folder clearly.
    QProcess zipProcess;
    zipProcess.setWorkingDirectory(tempDir.absolutePath() + "/..");
    QString tempDirName = tempDir.dirName();

    // Use -r to recurse, -q to reduce noise. Avoid options that may change text content.
...
bool GitBackend::applyBundle(const std::string &bundle_path, std::string &error_message)
{
    if (!isRepositoryOpen())
    {
        error_message = "No repository open to apply bundle to.";
        return false;
    }
    QProcess gitProcess;
    gitProcess.setWorkingDirectory(QString::fromStdString(m_currentRepoPath));
    ...
    if (!gitProcess.waitForFinished(-1))
    {
        error_message = "Git pull from bundle process timed out.";
        return false;
    }
